...
use serenity::all::{CommandInteraction, ComponentInteraction, CreateInteractionResponseMessage, ModalInteraction};
...
pub(crate) async fn handle_commands(ctx: &Context, interaction: CommandInteraction) {
    let result = match interaction.data.name.as_str() {
        "events" => create_event::handle(ctx, &interaction).await,
